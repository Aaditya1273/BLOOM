...
import { isSignRequest, type Account } from "@/lib/types";
    ...
    <div className="grid gap-10 lg:grid-cols-[minmax(0,1.55fr)_minmax(0,1fr)] lg:gap-14">
      ...
      <AmountDialog
        ...
        onSubmit={async (amount) => {
          ...
          const txHashes = isSignRequest(r) ? (await complete(r))!.txHashes : [r.txHash];
          return { txHashes, message: `${usd(amount)} is now in savings.` };
        }}
        onDone={refresh}
      />
      <AmountDialog
        ...
        onSubmit={async (amount) => {
          ...
          return {
            txHashes: isSignRequest(r) ? (await complete(r))!.txHashes : r.steps.map((s) => s.txHash).filter(Boolean),
